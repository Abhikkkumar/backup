...
router.get("/", (req, res) => {
  res.send("Hello, I am in auth.js");
});
  ...
  USER.findOne({ $or: [{ email: email }, { username: username }] }).then(
    (savedUser) => {
      ...
      bcrypt.hash(password, 12).then((hashedPass) => {
        ...
        user
          .save()
          .then((user) => res.status(200).json({ message: "Registered Successfully!" }))
...
router.post("/signin", (req, res) => {
  ...
  USER.findOne({ email: email }).then((savedUser) => {
    ...
    bcrypt
      ...
      .catch((err) => {return res.status(422).json("Error occured during signin!")});
  });
});
